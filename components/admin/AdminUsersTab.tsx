...
const AdminUsersTab: React.FC = () => {
  ...
  const [viewingRedemptionsForUser, setViewingRedemptionsForUser] =
  ...
  const handleVerifyUser = async (userId: string) => {
    if (
      window.confirm(
        t("confirmVerifyEmail") ||
          "Are you sure you want to manually verify this user's email?"
      )
    ) {
      try {
  ...
  const handleSendTestPush = async (userId: string) => {
    if (
      window.confirm(
        t("confirmSendPush") || "Send a test push notification to this user?"
      )
    ) {
      try {
  ...
  const handleResetHistory = async (userId: string) => {
    const confirmation = window.prompt(
      "WARNING: This will wipe ALL wallet items and redemption history for this user. Type 'RESET' to confirm:"
    );
  ...
  }, [page, searchQuery, tierFilter, statusFilter, sortBy, sortOrder]);
  ...
  const handleEditUserClick = async (user: User) => {
    ...
    try {
      const walletItems = await import("../../lib/supabaseService").then((m) =>
        m.getWalletItems(user.id)
      );
      const deals = walletItems
        .map((item: any) => item.deal)
        .filter((d: any) => d !== null);
    ...
    if (dealToAdd && editingUser) {
      try {
        ...
        if (dealObj) {
          ...
          const fetchedDeal = await import("../../lib/supabaseService").then(
            (m) => m.getDealById(dealToAdd)
          );
  ...
  const handleBulkAction = async (action: "email" | "activate" | "ban") => {
    ...
    if (action === "email") {
      alert(
        "Email bulk action is not yet implemented. This will integrated with your mailing service."
      );
      return;
    }
  ...
  return (
    <>
        ...
        {isUserFormVisible && (
          <section className="bg-white dark:bg-brand-surface p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 animate-fade-in">
            <h2 className="text-xl font-bold mb-6 text-gray-900 dark:text-white">
              {editingUser ? t("editUser") : "Add User"}
            </h2>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setRedemptionsToAdd(parseInt(e.target.value) || 0)
                      }
                ...
                <div className="space-y-2 mb-4 max-h-60 overflow-y-auto">
                  {userWalletDeals.length > 0 ? (
                    userWalletDeals.map((deal) => (
                      <div
                        key={deal.id}
                        className="flex justify-between items-center bg-white dark:bg-brand-surface p-3 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm"
                      >
                ...
                <div className="space-y-3">
                  <div className="relative">
                    <input
                      ...
                      className="w-full px-4 py-2 mb-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-brand-primary text-sm"
                    />
                    <div className="flex gap-3">
                      <select
                        value={dealToAdd}
                        onChange={(e) => setDealToAdd(e.target.value)}
                        className="flex-grow px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-brand-primary h-[42px]"
                      >
                      ...
                      <button
                        ...
                        disabled={!dealToAdd}
                        className="bg-brand-secondary text-white font-medium px-6 py-2 rounded-lg hover:bg-opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-sm h-[42px]"
                      >
                        {t("add")}
                      </button>
                    </div>
              ...
              <div className="pt-4 border-t border-gray-200 dark:border-gray-700 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <h3 className="text-lg font-semibold mb-2">
                    {t("referralChainLabel")}
                  </h3>
                  <div className="mb-2">
                    <label
                      htmlFor="referredBy"
                      className="block text-sm font-medium text-gray-600 dark:text-brand-text-muted mb-1"
                    >
                      Referred By (User ID or Select)
                    </label>
                    <div className="flex gap-2">
                      <input
                        type="text"
                  ...
                  <div className="bg-gray-100 dark:bg-brand-bg p-3 rounded-md text-sm text-gray-800 dark:text-brand-text-light min-h-[40px] flex items-center">
                    {(userFormData.referralChain?.length ?? 0) > 0 ? (
                      <span>
                        {userFormData.referralChain
                          ?.map((id) => userIdToNameMap[id] || "Unknown")
                          .join(" → ")}
                      </span>
                    ) : (
                      <p className="italic text-gray-500 dark:text-brand-text-muted">
                        {t("topOfChain")}
                      </p>
                    )}
                  </div>
                    ...
                    {(userFormData.referralNetwork?.length ?? 0) > 0 ? (
                      <ul className="list-disc list-inside space-y-1">
                        {userFormData.referralNetwork?.map((id) => (
                          <li key={id}>
                            {userIdToNameMap[id] || "Unknown User"}
                          </li>
                        ))}
                      </ul>
                    ) : (
          ...
          <div className="flex items-center gap-4 p-4 bg-gray-50 dark:bg-brand-bg rounded-lg border border-gray-200 dark:border-gray-700">
            <div className="flex-grow">
              ...
              <p className="text-sm text-gray-500 dark:text-brand-text-muted">
                Enable or disable notifications for ALL users. This overrides
                individual settings.
              </p>
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => {
                  if (
                    window.confirm(
                      "Are you sure you want to ENABLE notifications for ALL users?"
                    )
                  ) {
                    updateAllUsersNotificationPreferences({
              ...
              <button
                onClick={() => {
                  if (
                    window.confirm(
                      "Are you sure you want to DISABLE notifications for ALL users?"
                    )
                  ) {
                    updateAllUsersNotificationPreferences({
          ...
          <div className="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800 flex justify-between items-center">
            <div>
              ...
              <p className="text-sm text-blue-700 dark:text-blue-300">
                Send a push notification to all subscribed users immediately.
              </p>
            </div>
            <button
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            ...
            <div className="w-full md:w-48">
              <select
                ...
                className="w-full bg-white dark:bg-brand-surface border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-brand-primary focus:border-transparent"
              >
            ...
            <div className="w-full md:w-48 flex items-center gap-1 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-brand-surface px-2">
              <select
                value={sortBy}
                onChange={(e) => setSortBy(e.target.value)}
                className="flex-1 bg-transparent border-none py-2 text-sm text-gray-900 dark:text-white focus:ring-0"
              >
                  ...
                  {isLoading ? (
                    ...
                    paginatedUsers.map((user) => {
                      const { remaining, total } =
                        calculateRemainingRedemptions(user);
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {user.name}
                            {user.isAdmin && (
                              <span className="ml-2 text-xs bg-brand-secondary text-brand-bg font-bold px-2 py-0.5 rounded-full">
                                Admin
                              </span>
                            )}
                            {user.status === "banned" && (
                              <span className="ml-2 text-xs bg-red-500 text-white font-bold px-2 py-0.5 rounded-full">
                                Banned
                              </span>
                            )}
                          </th>
                          ...
                          <td className="px-6 py-4 text-right">
                            <div className="relative group inline-block">
                              <button className="flex items-center gap-1 text-gray-500 dark:text-gray-400 hover:text-brand-primary transition-colors font-medium">
                                <span>{t("actions")}</span>
                                <svg
                                  className="w-4 h-4"
                                  fill="none"
                              ...
                              <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 py-1 z-20 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                                <button
                                ...
                                >
                                  <span className="w-5 text-center">🎟️</span>{" "}
                                  {t("viewRedemptions") || "Redemptions"}
                                </button>
                                <button
                                  onClick={() => handleViewPaymentsClick(user)}
                                  className="w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2"
                                >
                                ...
                                <button
                                  onClick={() => handleViewActivityClick(user)}
                                  className="w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2"
                                >
                                  <span className="w-5 text-center">📜</span>{" "}
                                  Activity
      ...
      >
        <div className="p-4">
          {viewingRedemptionsForUser?.redemptions &&
          viewingRedemptionsForUser.redemptions.length > 0 ? (
            <div className="space-y-4">
              {viewingRedemptionsForUser.redemptions.map((redemption: any) => {
                const deal = allDeals.find((d) => d.id === redemption.dealId);
                return (
                  <div
                    key={redemption.id || Math.random()}
                    className="bg-gray-50 dark:bg-brand-bg p-3 rounded-lg border border-gray-100 dark:border-gray-700"
                  >
        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setBroadcastData((d) => ({ ...d, title: e.target.value }))
              }
              className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
          ...
          <div className="bg-yellow-50 dark:bg-yellow-900/20 p-3 rounded text-xs text-yellow-800 dark:text-yellow-200">
            ⚠️ This will result in real push notifications being sent to all
            users who have subscribed on their devices.
          </div>
      ...
      >
        <div className="p-4">
          ...
          <div className="mt-6 flex justify-end">
            <button
              onClick={() => setViewingPaymentsForUser(null)}
              className="bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-white font-semibold py-2 px-4 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors"
            >
              {t("close")}
            </button>
            ...
            <div className="relative border-l-2 border-gray-200 dark:border-gray-700 ml-3 space-y-6">
              {userActivityLog.map((item) => (
                <div key={item.id} className="mb-8 ml-6 relative">
                  <span
                    className={`absolute -left-9 flex items-center justify-center w-6 h-6 rounded-full ring-4 ring-white dark:ring-brand-surface ${
                      item.type === "joined"
                        ? "bg-blue-100 ring-blue-50"
                        : item.type === "deal_claimed"
                        ? "bg-yellow-100 ring-yellow-50"
                        : item.type === "deal_redeemed"
                        ? "bg-green-100 ring-green-50"
                        : item.type === "subscription_payment"
                        ? "bg-purple-100 ring-purple-50"
