  ...
  const socialPlatforms = [
    {
      ...
      action: `https://api.whatsapp.com/send?text=${encodeURIComponent(
        dealTitle + "\n" + shareUrl
      )}`,
      isLink: true,
      bg: "bg-[#25D366]",
    },
    {
      ...
      action: `https://t.me/share/url?url=${encodeURIComponent(
        shareUrl
      )}&text=${encodeURIComponent(dealTitle)}`,
      isLink: true,
      bg: "bg-[#0088cc]",
    },
    {
      name: "Facebook",
      icon: <FacebookLogo className="w-8 h-8 text-white" />,
      action: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(
        shareUrl
      )}`,
      isLink: true,
      bg: "bg-[#1877F2]",
      ...
      <div className="grid grid-cols-3 gap-6 py-4">
        {socialPlatforms.map((platform, index) => {
          const content = (
            <>
              <div
                className={`w-14 h-14 rounded-2xl ${platform.bg} flex items-center justify-center shadow-lg hover:scale-110 transition-transform duration-200`}
              >
                {platform.icon}
              </div>
              <span className="text-xs font-medium text-gray-700 dark:text-brand-text-muted text-center mt-2">
                {platform.name}
              </span>
            </>
          );
...
const DealDetailView: React.FC<DealDetailViewProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<"conditions" | "locations">(
    "conditions"
  );
  const [isLimitModalOpen, setIsLimitModalOpen] = useState(false);
  const [isWalletFullModalOpen, setIsWalletFullModalOpen] = useState(false);
  const [remainingRedemptions, setRemainingRedemptions] = useState<
    number | null
  >(null);
  const [pendingAction, setPendingAction] = useState<"redeem" | "claim" | null>(
    null
  );
  const [activeDealsCount, setActiveDealsCount] = useState(0);
  const [walletItemInfo, setWalletItemInfo] = useState<{
    id: string;
    redemptionCode: string;
  } | null>(null);
  const [localIsSoldOut, setLocalIsSoldOut] = useState(
    deal.isSoldOut ||
  ...
  const handleActionClick = (action: "redeem" | "claim") => {
    ...
    const dontShowAgain =
      localStorage.getItem("dontShowRedemptionWarning") === "true";
  ...
  const executeAction = async (action: "redeem" | "claim") => {
    ...
    try {
      ...
      if (
        ...
        errorMessage.includes("Could not determine")
      ) {
  ...
  const originalPriceFormatted =
    deal.originalPrice > 0 ? `₺${deal.originalPrice}` : "";
  ...
  return (
    <div className="bg-[#0f172a] min-h-screen relative pb-40 font-body text-white selection:bg-gold-500/30">
      ...
      <div className="absolute top-0 left-0 right-0 z-30 p-6 flex justify-between items-center bg-gradient-to-b from-black/60 to-transparent">
          ...
          >
            <div className="relative">
              <img
                src="/favicon.png"
                alt="Tripzy"
                className="w-8 h-8 object-contain drop-shadow-[0_0_8px_rgba(255,255,255,0.3)] group-hover:drop-shadow-[0_0_12px_rgba(212,175,55,0.5)] transition-all duration-500"
              />
            ...
            <div className="ml-2.5 flex flex-col">
              <span className="text-white font-black tracking-tighter text-xl leading-none uppercase italic">
                Tripzy
              </span>
              <span className="text-[8px] text-gold-400 font-bold tracking-[0.3em] uppercase opacity-80 leading-none mt-1">
                Travel Exclusive
              </span>
            </div>
          </div>
        ...
        <div className="flex gap-4">
          ...
          <button
            ...
            className={`w-12 h-12 flex items-center justify-center rounded-full backdrop-blur-md border shadow-lg hover:scale-105 transition-all duration-300 ${
              isDealSaved(deal.id)
          ...
          >
            <HeartIcon
              className={`h-6 w-6 ${
                isDealSaved(deal.id) ? "fill-current text-gold-200" : ""
              }`}
            />
          </button>
        </div>
      ...
      <div className="relative h-[60vh] md:h-[70vh] w-full overflow-hidden group">
        ...
        <img
          src={getHeroImageUrl(deal.imageUrl, deal.category)}
          ...
          onError={(e) => {
            (e.target as HTMLImageElement).src = getHeroImageUrl(
              null,
              deal.category
            );
      ...
      <div className="relative z-20 -mt-16 px-4 max-w-4xl mx-auto">
        <div className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-[2.5rem] shadow-[0_20px_40px_rgba(0,0,0,0.4)] p-8 relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-6 mb-10">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1.5">
                <span className="text-xs font-black uppercase tracking-[0.2em] text-gold-400/80">
                  Premium Partner
                </span>
                <div className="h-px flex-1 bg-gradient-to-r from-gold-500/30 to-transparent"></div>
              </div>
              <h2 className="text-4xl md:text-5xl font-heading font-black text-white leading-[1.1] mb-2 tracking-tight">
                {title}
              </h2>
          ...
          {localIsSoldOut ? (
            ...
            deal.maxRedemptionsTotal &&
            deal.maxRedemptionsTotal - (deal.redemptionsCount || 0) <= 10 && (
              <div className="inline-flex items-center gap-2 bg-orange-500/20 border border-orange-500/30 px-4 py-2 rounded-full mb-8 ml-3 shadow-inner animate-pulse">
                <span className="text-orange-200 font-bold text-sm tracking-wide">
                  🔥 {t("hurryOnly") || "Hurry! Only"}{" "}
                  {deal.maxRedemptionsTotal - (deal.redemptionsCount || 0)}{" "}
                  {t("left") || "Left!"}
                </span>
              </div>
            )
          ...
          <div className="flex p-1 bg-white/5 border border-white/10 rounded-2xl mb-10 relative">
            ...
            <button
              ...
              className={`flex-1 py-3.5 text-xs font-black tracking-[0.2em] uppercase transition-all duration-500 rounded-xl relative z-10 ${
                  ...
                  <div className="flex justify-center bg-white/5 rounded-2xl p-4 border border-white/5">
                    <StarRatingInput
                      ...
                      disabled={
                        ...
                        !user ||
                        (user &&
                          !user.redemptions?.some((r) => r.dealId === deal.id))
                      }
                    />
                  </div>
                </div>
                    ...
                    {deal.latitude && deal.longitude ? (
                      <div className="w-full space-y-6">
                          ...
                          {import.meta.env.VITE_GOOGLE_MAPS_API_KEY ? (
                            <iframe
                              ...
                              src={`https://www.google.com/maps/embed/v1/place?key=${
                                import.meta.env.VITE_GOOGLE_MAPS_API_KEY
                              }&q=${deal.latitude},${deal.longitude}&zoom=19`}
                              allowFullScreen
                              loading="lazy"
                        ...
                        <button
                          onClick={() =>
                            window.open(
                              `https://www.google.com/maps/place/${deal.latitude},${deal.longitude}/@${deal.latitude},${deal.longitude},19z`,
                              "_blank"
                            )
                          }
              ...
              {[
                {
                  ...
                  title: t("step1"),
                  desc: t("step1Desc"),
                  icon: <CustomBriefcaseIcon className="w-6 h-6" />,
                },
                {
                  step: 2,
                  title: t("step2"),
          ...
          <div className="mt-16 grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              ...
              {
                label: t("noHiddenFees"),
                icon: <CheckCircle className="w-4 h-4 text-emerald-400" />,
              },
              {
                label: t("guaranteedAccess"),
                icon: <CheckCircle className="w-4 h-4 text-emerald-400" />,
              },
            ].map((item, idx) => (
              <div
                key={idx}
                className="flex items-center gap-3 bg-white/5 border border-white/5 px-5 py-4 rounded-2xl"
              >
                {item.icon}
          ...
          <div className="glass-premium rounded-[1.25rem] p-2 md:p-3 flex items-center justify-between gap-3 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] border-t border-white/20">
            ...
            <div className="flex flex-col min-w-fit pl-1">
              <div className="flex items-center gap-2 mb-0.5">
                {deal.originalPrice > 0 ? (
                  <>
                    <span className="text-white/40 text-sm line-through decoration-white/30">
                      ₺{deal.originalPrice}
                    </span>
              ...
              <div className="text-2xl font-black text-white tracking-tighter flex items-baseline gap-1">
                ...
                <span className="text-[10px] font-medium text-white/30 ml-1 uppercase tracking-widest">
                  {t("total") || "per person"}
                </span>
              </div>
            </div>
              ...
              {localIsSoldOut ? (
                ...
                <>
                  ...
                  {isLocked ? (
                    ...
                    <button
                      onClick={() =>
                        isPreview ? null : handleActionClick("redeem")
                      }
        ...
        <div className="p-4">
          ...
          <div className="flex items-center mb-6">
            ...
            <label
              ...
              className="ml-2 text-sm text-gray-600 dark:text-brand-text-muted"
            >
              {t("dontShowAgain")}
            </label>
          </div>
          <div className="flex gap-4">
            ...
            >
              {pendingAction === "claim"
                ? t("confirmAddToWallet") || "Confirm"
                : t("confirmRedeem") || "Redeem"}
            </button>
          </div>
        </div>
      </Modal>
      ...
      >
        <div className="text-center">
          <p className="text-sm text-gray-600 dark:text-brand-text-muted mb-4">
            {t("showThisCodeAtCheckout")}
          </p>
          {walletItemInfo ? (
            <WalletQRCode
            ...
            <div className="bg-gradient-to-br from-brand-primary/10 to-brand-secondary/10 border-2 border-dashed border-brand-primary/30 rounded-xl py-8 px-4 mb-6">
              <p className="text-4xl font-mono font-bold tracking-widest text-gradient mb-2">
                {deal.redemptionCode}
              </p>
              <p className="text-xs text-gray-500">
                Legacy code - add to wallet for secure QR
              </p>
            </div>
          )}
          <button
      ...
      >
        <div className="space-y-4 text-sm text-gray-700 dark:text-brand-text-muted">
          <div>
            <h4 className="font-semibold text-gray-900 dark:text-brand-text-light mb-2">
              {t("usageLimit")}
            </h4>
      ...
      >
        <div className="p-6 text-center">
          ...
          <p className="text-gray-600 dark:text-brand-text-muted mb-6">
            {t("upgradePrompt") ||
              "You have used all your redemptions for this month. Upgrade your plan to unlock more amazing deals!"}
          </p>
      ...
      >
        <div className="p-6 text-center">
          ...
          <p className="text-gray-600 dark:text-brand-text-muted mb-2">
            {t("walletLimitReachedDesc") ||
              "Please upgrade your plan or remove some deals to add more."}
          </p>
          <p className="text-sm text-gold-400 mb-6">
            {`${activeDealsCount} / ${getWalletLimit(
              user?.tier || SubscriptionTier.FREE,
              user?.walletLimit
            )} ${language === "tr" ? "fırsat cüzdanda" : "deals in wallet"}`}
          </p>
          <div className="flex flex-col gap-3">
            <button
              onClick={() => {
