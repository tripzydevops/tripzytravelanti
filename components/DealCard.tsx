...
const StarRating: React.FC<{
  ...
  t: (key: string) => string;
}> = ({ rating = 0, ratingCount = 0, t }) => {
  const numRating = Number(rating) || 0;
  const numCount = Number(ratingCount) || 0;

  if (numCount === 0) {
    return <p className="text-xs text-slate-400">{t("noRatingsYet")}</p>;
  }

  const fullStars = Math.min(5, Math.max(0, Math.floor(numRating)));
  ...
  return (
    <div className="flex items-center">
      <div className="flex items-center mr-1.5">
        ...
        {[...Array(Math.max(0, 5 - fullStars))].map((_, i) => (
          <StarIcon key={`empty-${i}`} className="w-3.5 h-3.5 text-amber-400/30" />
        ))}
      </div>
      <span className="text-xs font-bold text-slate-300">
        {numRating.toFixed(1)} <span className="font-normal text-slate-400">({numCount})</span>
      </span>
    </div>
  );
};
...
const DealCard: React.FC<DealCardProps> = ({ deal }) => {
  ...
  const ratingCount = Number(deal.ratingCount) || 0;

  // Multi-image gallery list with fallback
  const galleryImages = useMemo(() => {
    if (deal.galleryUrls && Array.isArray(deal.galleryUrls) && deal.galleryUrls.length > 0) {
      const valid = deal.galleryUrls.filter(Boolean);
  ...
  const userTierLevel = user
    ? TIER_LEVELS[user.tier] ?? 0
    : TIER_LEVELS[SubscriptionTier.NONE] ?? 0;
  const requiredTierLevel = TIER_LEVELS[deal.requiredTier] ?? 1;

  // Guests see FREE tier deals as unlocked, but higher tiers as locked.
  let isLocked = userTierLevel < requiredTierLevel;
  if (!user && (deal.requiredTier === SubscriptionTier.FREE || !deal.requiredTier)) {
    isLocked = false;
  }

  const title = language === "tr"
    ? (deal.title_tr || deal.title || "Fırsat")
    : (deal.title || deal.title_tr || "Deal");

  const vendorName = deal.vendor || "Tripzy Partner";
  const vendorInitial = (vendorName.charAt(0) || "T").toUpperCase();
  ...
  const netSavings = originalPrice > discountedPrice ? originalPrice - discountedPrice : 0;

  const calculateDaysLeft = (expiryDate?: string) => {
    if (!expiryDate) return t("neverExpires");
    const now = new Date();
    const expiry = new Date(expiryDate);

    if (isNaN(expiry.getTime()) || expiry.getFullYear() > now.getFullYear() + 50) {
  ...
  const aiMatchScore = deal.aiMatchScore || (rating >= 4.0 ? 96 : 92);
  const aiMatchReason = language === 'tr'
    ? (deal.aiMatchReason_tr || `Seyahat stiliniz ve ${deal.category_tr || deal.category || 'fırsat'} tercihlerinizle eşleşti.`)
    : (deal.aiMatchReason || `Curated for your travel style and preferences in ${deal.category || 'deals'}.`);

  const currentImageUrl = galleryImages[activeImageIndex] || galleryImages[0];

  const CardContent = () => (
    <>
      {/* Image Container with In-Card Carousel & Double Tap */}
      <div
      ...
      >
        ...
        <img
          ...
          src={getThumbnailUrl(currentImageUrl, deal.category)}
          alt={title}
          loading="lazy"
          decoding="async"
        />
        ...
        <div className="absolute top-3 left-3 z-20 flex flex-wrap gap-1.5 items-center max-w-[75%]">
          {discount > 0 && !isLocked && (
        ...
        {isLocked && (
          <div className="absolute inset-0 bg-black/60 backdrop-blur-md flex flex-col items-center justify-center text-white p-4 text-center z-20">
            ...
            <span className="text-xs font-semibold text-slate-200 max-w-[170px]">
              {!user
                ? t("loginToUnlock")
                : `${deal.requiredTier || 'PREMIUM'} ${t("toUnlock")}`}
            </span>
          </div>
        )}
      ...
      {/* Content Section with High Outdoor Legibility */}
    ...
    <>
      ...
      {isShareModalOpen && (
        <SocialShareModal
          ...
          shareType="deal"
          title={title}
          subtitle={`${vendorName} • ${discount > 0 ? `%${discount} İndirim` : ''}`}
          discountPercentage={discount}
          dealImageUrl={currentImageUrl}
