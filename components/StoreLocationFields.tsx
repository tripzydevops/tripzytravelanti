...
const StoreLocationFields: React.FC<StoreLocationFieldsProps> = ({
    ...
    const isOnlineOnly = redemptionStyle.includes('online') && !redemptionStyle.includes('in_store');
    const isInStore = redemptionStyle.includes('in_store');
    const isBoth = redemptionStyle.includes('online') && redemptionStyle.includes('in_store');
    ...
    const handleRemoveLocation = (index: number) => {
        ...
        onChange(updated.length > 0 ? updated : [{ name: '', address: '', city: '' }]);
    };

    const handleLocationChange = (index: number, field: keyof StoreLocation, value: string) => {
        const updated = storeLocations.map((loc, i) =>
    ...
    const locations = storeLocations.length > 0 ? storeLocations : [{ name: '', address: '', city: '' }];
        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-gray-700 dark:text-gray-300">Store Locations</span>
                </div>
                {isBoth && (
